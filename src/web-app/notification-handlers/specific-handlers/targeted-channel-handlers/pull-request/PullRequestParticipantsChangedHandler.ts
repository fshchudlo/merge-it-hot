...
export class PullRequestParticipantsChangedHandler
    ...
    public async handle(
        payload: PullRequestParticipantsUpdatedEvent,
...
async function updateChannelMembers(
    ...
    const userIdsToAdd = payload.addedParticipants.map(
        payload => payload.slackUserId
    );
