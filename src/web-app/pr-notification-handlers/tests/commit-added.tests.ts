...
describe("Commit added use-case", () => {
    ...
    it("Should invite the committer to the channel if they are not participate yet", async () => {
        ...
        await handlePullRequestEvent(TestPayloadBuilder.pullRequestFromRefUpdated(authorUser), channelMock);

        expect(channelMock.snapshot.invitesToChannels).toEqual([]);

        await handlePullRequestEvent(TestPayloadBuilder.pullRequestFromRefUpdated(nonReviewerUser), channelMock);
