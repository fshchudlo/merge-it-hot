...
function getReviewerActionDescription(
    payload: PullRequestReviewSubmittedEvent,
) {
    const prLink = link(payload.pullRequest.links.self, "pull request");
    const commentPart = payload.review.comment
...
function getReviewStatusBlocks(pullRequest: PullRequestPayload) {
    ...
    const whoApproved = pullRequest.participants
        .filter(r => r.status == "APPROVED")
        .map(r => r.user.name);
    const whoRequestedWork = pullRequest.participants
        .filter(r => r.status == "NEEDS_WORK")
        .map(r => r.user.name);
    const whoUnapproved = pullRequest.participants
    ...
    if (whoRequestedWork.length == 0 && whoUnapproved.length == 0) {
        ...
        const reviewStatuses = [
            ` Approved: ${whoApproved.length == 0 ? "0" : whoApproved.join(",")}`,
        ];
        whoRequestedWork.length > 0
            ? reviewStatuses.push(
                  ` Requested changes: ${whoRequestedWork.join(",")}`,
              )
            : "";
        whoUnapproved.length > 0
            ? reviewStatuses.push(` Not reviewed: ${whoUnapproved.join(",")}`)
            : "";
