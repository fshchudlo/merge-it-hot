import { PullRequestPayload } from "../../web-app/pr-notification-handlers/event-contracts";
...
export function buildChannelName(params: PullRequestFlattenTraits | PullRequestPayload): string {
    ...
    const pullRequestId = ("pullRequestId" in params ? params.pullRequestId : params.number).toString();
    const projectKey = ("projectKey" in params ? params.projectKey : params.targetBranch.projectKey).replace("~", "").trim();
    const repositorySlug = ("repositorySlug" in params ? params.repositorySlug : params.targetBranch.repositoryName).replaceAll(".", "-");
