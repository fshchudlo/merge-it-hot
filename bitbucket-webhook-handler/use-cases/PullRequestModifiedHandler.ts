...
export class PullRequestModifiedHandler implements PullRequestNotificationHandler {
    ...
    public async handle(payload: PullRequestModifiedNotification, slackChannel: SlackChannel) {
        await this.sendMessageAboutPRModification(payload, slackChannel);
    }

    private async sendMessageAboutPRModification(payload: PullRequestModifiedNotification, slackChannel: SlackChannel) {
        const visibleChanges = this.getChangesDescription(payload);
    ...
    private getChangesDescription(payload: PullRequestModifiedNotification) {
